...
export async function GET(
  ...
  try {
    console.log("[SG][certificate-summary] fetching", { rrn, base: UKG_BASE });

    const token = await getAccessToken();

    const upstream = await fetch(
      `${UKG_BASE}/scotland/assessments/${encodeURIComponent(rrn)}/certificate-summary`,
      {
        headers: {
    ...
    if (!upstream.ok) {
      console.error("[SG][certificate-summary] upstream error", {
        rrn,
    ...
    console.log("[SG][certificate-summary] success", {
      rrn,
      bytes: txt.length,
    });
    ...
    const msg = errMsg(e);
    console.error("[SG][certificate-summary] route failure", { rrn, msg });
